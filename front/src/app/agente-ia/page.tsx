            ...
            if (cantidad) {
                ...
                if (result.success) {
                    setChat((prev) => [
                        ...
                        {
                            actor: "ai",
                            txt: `¡Entendido! He procesado automáticamente el descuento. ${result.message}`
                        }
                    ]);
                } else {
                    setChat((prev) => [
                        ...
                        {
                            actor: "ai",
                            txt: `Lo siento, hubo un problema operativo: ${result.message}`
                        }
                    ]);
                }
            } else {
        ...
        <div className="glass-panel flex flex-col h-[75vh] max-w-4xl mx-auto overflow-hidden border border-purple-500/30">
            <div className="bg-purple-900/40 p-6 border-b border-purple-500/30 flex gap-4 items-center">
                ...
                <div>
                    ...
                    <p className="text-sm text-purple-300/70 mt-1">
                        Simulación de Auto-Surtido de Stock (Local JS)
                    </p>
                </div>
            </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-6">
                {chat.map((msg, i) => (
                    <div
                        key={i}
                        className={`flex ${msg.actor === "user" ? "justify-end" : "justify-start"}`}
                    >
                        <div
            ...
            <form
                onSubmit={handleSend}
                className="p-5 bg-gray-900 border-t border-gray-800 flex gap-3 items-center"
            >
                <input
                    type="text"
                    value={input}
